...
html_file = 'matrimonial.html'
with open(html_file, 'r', encoding='utf-8') as f:
    html = f.read()

# Update search input
search_old = r'<input type="text" class="input-field" placeholder="Search by Gotra, Profession\.\.\." style="flex: 2; min-width: 200px;">'
search_new = '<input type="text" id="searchProfile" class="input-field" placeholder="Search by Name, Gotra, Profession..." style="flex: 2; min-width: 200px;">'
html = re.sub(search_old, search_new, html)

# Replace the entire profile-grid with a loader and empty grid
grid_old = r'<div class="profile-grid".*?</div>\s*</div>\s*<!-- Register Profile Tab -->'
grid_new = '''<div id="profiles-loader" style="text-align: center; padding: 50px; color: var(--primary-color);">
                    <i class="fas fa-spinner fa-spin fa-3x"></i>
                    <p style="margin-top: 15px; font-weight: bold;"><span class="lang-hi">प्रोफाइल लोड हो रहे हैं...</span><span class="lang-en">Loading profiles...</span></p>
                </div>
                <div id="profiles-grid" class="profile-grid" style="display: none; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px;">
...
html = re.sub(grid_old, grid_new, html, flags=re.DOTALL)

with open(html_file, 'w', encoding='utf-8') as f:
    f.write(html)
print("Updated matrimonial.html")


js_file = 'matrimonial.js'
js_content = '''
let allProfiles = [];
